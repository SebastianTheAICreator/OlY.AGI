...
def main():
    ...
    try:
        ...
        model_config = OLyConfig(
            block_size=256,        # Mărit pentru răspunsuri lungi
            vocab_size=50257,      
            n_layer=6,            # Redus pentru dataset mic
            n_head=8,            
            n_embd=512,           # Dimensiune embedding potrivită
            dropout=0.1,          
            multiple_of=128,
            bias=True,
            use_moe=False,         # Dezactivat pentru simplitate
            num_experts=1,         
            expert_capacity=32,   
        ...
        training_config = TrainingConfig(
            batch_size=1,            # Dataset foarte mic
            epochs=100,               # Redus pentru evitare overfitting
            learning_rate=5e-5,      # Learning rate conservator
            weight_decay=0.2,        # Regularizare puternică
            warmup_steps=10,         # Warmup redus pentru dataset mic
            eval_interval=5,         # Evaluare foarte frecventă
            save_interval=15,         
            grad_clip=0.3,          # Restrictiv
            mixed_precision=True,
            gradient_accumulation_steps=4,  # Acumulare pentru stabilitate
            early_stopping_patience=150,  # Early stopping agresiv
...
# Fac modelul sa vorbeasca in Romana...
