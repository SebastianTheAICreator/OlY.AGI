...
def main():
    try:
        ...
        model_config = OLyConfig(
            block_size=512,          # Mărit pentru context mai bun
            vocab_size=50257,        
            n_layer=12,               
            n_head=16,               # Mărit pentru atenție mai bună
            n_embd=1024,            # Mărit pentru reprezentări mai bogate
            dropout=0.2,            # Mărit pentru regularizare mai bună
            use_moe=True,
            num_experts=8,          # Mărit pentru complexitate
            expert_capacity=128,    # Mărit pentru procesare mai bună
            use_multiquery=True,
            use_rotary=True,
            ...
            memory_bank_size=1000   # Mărit pentru memorare mai bună
        )

        training_config = TrainingConfig(
            batch_size=8,           # Redus pentru stabilitate
            epochs=100,               
            learning_rate=5e-5,     # Redus pentru convergență mai bună
            weight_decay=0.005,     # Redus pentru stabilitate
            warmup_steps=1000,      # Mărit pentru încălzire graduală
            ...
            adam_beta2=0.95,
            adam_epsilon=1e-8,
            max_grad_norm=1.0,       # Ajustat pentru stabilitate
            lr_scheduler_type="cosine",
            num_cycles=1.0          # Redus pentru learning rate mai stabil
        )
        ...
        try:
            ...
            print(f"\nShape verificare:")
            print(f"  ├── Input: {sample['input_ids'].shape}")
            print(f"  ├── Target: {sample['target_ids'].shape}")
            print(f"  └── Mask: {sample['attention_mask'].shape}")
        ...
        try:
            ...
            # Test generare cu parametri îmbunătățiți
            test_cases = [
                {
