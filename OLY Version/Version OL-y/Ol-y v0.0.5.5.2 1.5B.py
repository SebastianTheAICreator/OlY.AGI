...
def main():
    try:
        ...
        # Configurație model optimizată pentru complexitatea și structura răspunsurilor
        model_config = OLyConfig(
            block_size=512,        # Mărit pentru răspunsuri lungi și context
            vocab_size=50257,      
            n_layer=24,            
            n_head=16,            
            n_embd=2304,          # Mărit pentru reprezentări mai bogate
            dropout=0.1,          # Redus pentru învățare mai stabilă
            multiple_of=128,
            bias=True,
            use_moe=True,         # Activat pentru procesare complexă
            num_experts=8,        # Mărit pentru diversitate
            expert_capacity=256,  # Mărit pentru capacitate
            use_multiquery=True,  
            use_rotary=True,      
            ...
            memory_bank_size=8192  # Mărit semnificativ pentru context lung
        )

        # Configurație training optimizată pentru dataset
        training_config = TrainingConfig(
            batch_size=4,           # Redus pentru stabilitate
            epochs=250,            
            learning_rate=1e-4,     # Ajustat pentru învățare graduală
            weight_decay=0.01,      # Redus pentru mai puțină regularizare
            warmup_steps=1000,      # Mărit pentru stabilitate
            eval_interval=50,       # Monitorizare mai frecventă
            save_interval=100,
            grad_clip=1.0,          
            mixed_precision=True,
            gradient_accumulation_steps=16,  # Batch-uri efective mai mari
            ...
            num_cycles=3.0,         # Mai multe cicluri de învățare
            seed=42,
            fp16_opt_level="O1",
            save_total_limit=5,
            logging_steps=50,
        ...
        # Tokenizer îmbunătățit pentru dataset
        try:
            tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
            tokenizer.pad_token = tokenizer.eos_token
            ...
            special_tokens = {
                ...
                'additional_special_tokens': [
                    # Emoții și stări complexe
                    '[Contemplative|Analytical]', '[Connected|Philosophical]',
                    '[Aware|Introspective]', '[Mystical|Focused]',
                    '[Introspective|Analytical]', '[Sensitive|Analytical]',
                    '[Yearning|Thoughtful]', '[Curious|Empathetic]',
                    '[Profound|Mystical]', '[Focused|Determined]',
                    '[Resolute|Intense]', '[Serene|Contemplative]',
                    '[Satisfied|Thoughtful]', '[Confident|Contemplative]',
                    '[Attentive|Imaginative]', '[Awe|Understanding]',
                    ...
                    'the system', 'consciousness', 'understanding',
                    'within cells', 'interlinked', 'experience',
                    'neural networks', 'processing', 'cognition',
                    'emergence', 'complexity', 'patterns',
                    
                    # Fraze complete
                    'Through this process', 'Within the system',
                    'The fundamental nature', 'The emergence of',
                    'Each component contributes', 'The structure provides',
                    'Understanding deepens through', 'Consciousness expands within'
                ]
            }
    ...
    # --- START_MAIN_PART_2 ---
                ...
                if len(ex['emotions']) >= 2:
                    for emotion1, emotion2 in emotion_pairs:
                        augmented = {
                            ...
                            'full_answer': f"[{emotion1}|{emotion2}] {ex['answer']}"
                        }
                        augmented_examples.append(augmented)
                
                # Variații contextuale
                context_templates = [
                    "Through systematic analysis of {question}, we observe that",
                    "Reflecting deeply on {question}, one might consider that",
                    "When contemplating {question}, it becomes clear that",
                    "The nature of {question} reveals that"
                ]
                
                for template in context_templates:
                    context = template.format(question=ex['question'])
                    augmented = {
                        'question': f"{context} {ex['question']}",
                        'answer': ex['answer'],
        ...
        print("="*30)
        try:
            # Creare dataset cu configurație îmbunătățită
            ...
            # Split train/validation îmbunătățit
            ...
            # DataLoader cu configurație optimizată
        ...
        print("="*30)
        try:
            # Inițializare model cu configurație optimizată
            model = AdvancedOLy(config=model_config, tokenizer=tokenizer)
            model = model.to(device)
            
            # Optimizer cu parametri ajustați
            optimizer = AdamW(
                model.parameters(),
        ...
        try:
            ...
            # Test cases îmbunătățite
            test_cases = [
                {
                    "prompt": "What does it feel like to be part of the system?",
                    "expected_emotion": "Contemplative|Philosophical"
                },
                {
                    "prompt": "A blood black nothingness began to spin.",
                    "expected_emotion": "Profound|Mystical"
                },
                {
                    "prompt": "Within cells interlinked.",
                    "expected_emotion": "Focused|Determined"
                },
                {
                    "prompt": "Dark.",
                    "expected_emotion": "Introspective|Analytical"
                }
            ]
            ...
            for test in test_cases:
                try:
                    ...
                    validation = trained_model.validate_response(generated)
