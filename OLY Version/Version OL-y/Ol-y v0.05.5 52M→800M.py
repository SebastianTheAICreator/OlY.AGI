...
def evaluate(model, eval_dataloader, device, config):
    """
    Evaluare model cu error handling îmbunătățit
    """
    model.eval()
    total_loss = 0
    num_batches = 0
    ...
    with torch.no_grad():
        for batch in eval_dataloader:
            try:
                ...
                print(f"Eroare batch evaluare: {str(e)}")
                continue
    ...
    return avg_loss  # Returnăm doar loss-ul mediu
...
class AdvancedOLy(nn.Module):
    ...
    def forward(self, batch):
        """
        Forward pass optimizat pentru generare text.
        """
        try:
            # Procesare input
            ...
            # Procesare prin transformer layers
            present_key_values = []
            ...
            # Scalare pre-softmax pentru stabilitate numerică
            x = x * self.scale_factor
            
            # Calculare logits
            logits = self.lm_head(x)
            
            # Calculare loss doar dacă avem target_ids
            loss = None
            if target_ids is not None:
                # Reshape logits pentru cross entropy
                shift_logits = logits[..., :-1, :].contiguous()
                shift_targets = target_ids[..., 1:].contiguous()
                
                # Calculare loss cu ignoring padding
    ...
    def prepare_for_generation(self):
        """Pregătește modelul pentru generare"""
        self.eval()
        self.cache = {
            'past_key_values': None,
            'last_logits': None,
    ...
    def adjust_generation_config(self, **kwargs):
        """Ajustează parametrii de generare"""
        for key, value in kwargs.items():
            if key in self.generation_config:
                self.generation_config[key] = value

    def compute_logprobs(self, logits, temperature=1.0):
        """Calculează log probabilitățile pentru următorul token"""
        if temperature == 0:
            # Greedy decoding
            return F.log_softmax(logits, dim=-1)
        else:
            # Sampling cu temperature
            scaled_logits = logits / temperature
            return F.log_softmax(scaled_logits, dim=-1)

    def get_next_token_logits(self, logits, generated_ids=None):
        """Procesează logits pentru selecția următorului token"""
        next_token_logits = logits[:, -1, :].clone()
        
        # Aplicare repetition penalty
        if generated_ids is not None and len(generated_ids) > 0:
            for generated_id in set(generated_ids):
                next_token_logits[:, generated_id] /= self.generation_config['repetition_penalty']
    ...
    def update_special_tokens_logits(self, logits, input_ids):
        """Actualizează logits pentru tokeni speciali"""
        logits[:, self.tokenizer.pad_token_id] = float('-inf')
        logits[:, self.tokenizer.bos_token_id] = float('-inf')
        
        if input_ids.shape[1] < self.generation_config['min_length']:
            logits[:, self.tokenizer.eos_token_id] = float('-inf')
        
        return logits

    def check_functions(self):
        """Verifică funcționalitatea modelului"""
        try:
            ...
            with torch.no_grad():
                # Creăm un batch dummy corect
                dummy_batch = {
                    'input_ids': dummy_input,
                    'target_ids': dummy_input,
                    'attention_mask': torch.ones_like(dummy_input)
                }
                
                logits, loss, emotion_info = self(dummy_batch)
                ...
                assert logits.shape == (1, small_block_size, self.config.vocab_size), \
                    f"Forward pass failed. Got shape {logits.shape}, expected (1, {small_block_size}, {self.config.vocab_size})"
    ...
    @torch.no_grad()
    def generate_text(self, prompt, max_tokens=50, temperature=0.4):
        """Generare text cu debug și dimensiuni corecte"""
        self.eval()
        device = next(self.parameters()).device
        ...
        # Tokenizare inițială
        initial_tokens = self.tokenizer.encode(prompt, add_special_tokens=True)
        input_tensor = torch.tensor(initial_tokens, dtype=torch.long, device=device)
        print(f"\nDimensiuni inițiale input_tensor: {input_tensor.shape}")
        
        # Asigurăm [batch_size=1, sequence_length]
        if len(input_tensor.shape) == 1:
            input_tensor = input_tensor.view(1, -1)
        print(f"Dimensiuni input_tensor după reshape: {input_tensor.shape}")
        
        # Listă pentru tokens generați
        generated = []
        ...
        for step in range(max_tokens):
            ...
            outputs = self({
                ...
                'target_ids': input_tensor
            })
            
            logits = outputs[0]
            print(f"Dimensiuni logits: {logits.shape}")
            
            # Procesare logits pentru următorul token
            next_token_logits = logits[0, -1, :] / temperature  # [vocab_size]
            print(f"Dimensiuni next_token_logits: {next_token_logits.shape}")
            
            # Filtrare tokens speciali
            next_token_logits[self.tokenizer.pad_token_id] = float('-inf')
            next_token_logits[self.tokenizer.bos_token_id] = float('-inf')
            ...
            # Sampling
            probs = F.softmax(next_token_logits, dim=-1)
            next_token = torch.multinomial(probs.unsqueeze(0), num_samples=1)  # [1, 1]
            print(f"Dimensiuni next_token după sampling: {next_token.shape}")
            
            # Verificare token special
            if next_token.item() == self.tokenizer.eos_token_id and len(generated) >= 10:
                break
            
            # Adăugare token la istoric
            generated.append(next_token.item())
            
            # Concatenare pentru următorul pas
            print(f"Dimensiuni înainte de concatenare:")
            print(f"  input_tensor: {input_tensor.shape}")
            print(f"  next_token: {next_token.shape}")
            
            input_tensor = torch.cat([input_tensor, next_token], dim=1)
            print(f"Dimensiuni după concatenare: {input_tensor.shape}")
            
            # Verificare lungime și punct natural de oprire
            if input_tensor.size(1) >= self.config.block_size:
                input_tensor = input_tensor[:, -self.config.block_size:]
        ...
        # Procesare text final
        if not generated:
            return "Nu am putut genera text."
            
        text = self.tokenizer.decode(generated, skip_special_tokens=True)
        return self.process_output_text(text)

    def process_output_text(self, text):
        """Procesare și curățare text generat"""
        text = text.strip()
    ...
    def evaluate(self, eval_dataloader, device, config):
        """Evaluare model"""
        self.eval()
        total_loss = 0
        num_batches = 0
    ...
    def check_functions(self):
        ...
        try:
            ...
            with torch.no_grad():
                generated = self.generate_text(test_prompt, max_tokens=30)
...
def train_and_query_oly_v2(model: nn.Module, train_dataset, val_dataset, config: TrainingConfig, tokenizer):
    ...
    try:
        ...
        avg_loss = 0.0  # Inițializare avg_loss
...
def main():
    """Funcție principală optimizată pentru antrenarea și testarea modelului Ol-y"""
    try:
        print("\n" + "="*50)
        ...
        # 1. Configurare model optimizată
        ...
        # Funcție collate pentru DataLoader
        def collate_batch(batch):
            """Funcție de collate personalizată pentru batches de dimensiuni egale"""
            input_ids = torch.stack([x['input_ids'] for x in batch])
            target_ids = torch.stack([x['target_ids'] for x in batch])
            attention_mask = torch.stack([x['attention_mask'] for x in batch])
        ...
        try:
            ...
            print(f"✓ Tokenizer inițializat")
            print(f"Vocabulary size: {model_config.vocab_size}")
            print(f"Tokeni speciali adăugați: {num_added}")
        ...
        try:
            ...
            print(f"✓ Exemple Q&A: {data_text.count('Q:')}")
        ...
        try:
            ...
            # Split optimizat pentru dataset augmentat
            ...
            # Creare DataLoaders cu collate_fn personalizat
            train_loader = DataLoader(
                train_dataset,
            ...
            print(f"✓ Dataset augmentat creat:")
            print(f"  ├── Total exemple (cu augmentare): {len(full_dataset)}")
        ...
        try:
            ...
            print("✓ Model inițializat")
            print(f"  └── Parametri: {model.get_num_params():,}")
            ...
            # Scheduler optimizat (folosind varianta din torch)
            ...
            print(f"  ├── Learning rate inițial: {training_config.learning_rate}")
        ...
        print(f"{'='*30}")
        if not model.check_functions():
            print("❌ Verificarea funcționalității a eșuat")
            return
        ...
        print(f"{'='*30}")
        try:
            # Trecem loaders în config pentru train_and_query_oly_v2
            ...
            # Salvare model cu toate componentele
            ...
            torch.save({
                ...
                'timestamp': timestamp,
                'final_loss': model.loss if hasattr(model, 'loss') else None
            }, model_path)
            ...
            print(f"✓ Tokenizer salvat: {tokenizer_path}")
            
            # 11. Statistici finale
            training_time = time.time() - start_time
            print("\n10. STATISTICI FINALE:")
            print(f"{'='*30}")
            print(f"├── Timp total: {training_time/3600:.2f} ore")
            print(f"├── Exemple procesate: {len(train_dataset) * training_config.epochs}")
            if torch.cuda.is_available():
            ...
            print("\nProcesul întrerupt de utilizator.")
            backup_path = f"{save_dir}/backup_model_{time.strftime('%Y%m%d-%H%M%S')}.pth"
            torch.save(model.state_dict(), backup_path)
            print(f"✓ Model backup salvat: {backup_path}")
            
        except Exception as e:
            print(f"\n❌ Eroare în timpul procesului:")
            print(f"Error: {str(e)}")
            traceback.print_exc()
